...
if (!session_start()) {
    ...
    $sno = $_SESSION['stno'];
    $getvotequery = "SELECT votedtrea FROM studentvote WHERE sno = '$sno'";
    $vote = mysqli_query($conn, $getvotequery);
    $data = mysqli_fetch_array($vote);
    $status = $data['votedtrea'];
    if ($status == 0) {
        if (isset($_POST['votetrea'])) {
            $id = $_POST['votetrea'];
            $votequery = "SELECT votes FROM trea WHERE trea_no = '$id'";
            $vote = mysqli_query($conn, $votequery);
            $data = mysqli_fetch_array($vote);
            $getvote = $data['votes'];
            $getvote = $getvote + 1;
            $updatevote = "UPDATE trea SET votes = '$getvote' WHERE trea_no = '$id'";
            mysqli_query($conn, $updatevote);
            $updatestudent = "UPDATE studentvote SET votedtrea = '$id' WHERE sno = '$sno'";
...
<body>
    ...
    <div>
        <?php
        include('connection.php');
        $countquery = "SELECT * FROM candidate WHERE candidateposition = 'Treasurer'";
                    ...
                    <div class="col-6 card text-center" style="width: 18rem;">
                            <img src="src/candidate/Treasurer/' . $imageurl . '" class="card-img-top py-3 rounded-circle" alt="...">
                             <div class="card-body py-0 px-0">
                                ...
                                <button class="btn btn-primary mb-2" type="submit" name = "votetrea" value = "' . $cstno . '" >Vote ' . $cname . ' </button>
                            </div>
                        </div>
                    </div>
                </form>';
