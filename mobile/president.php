    ...
    if (!session_start()) {
        ...
        $sno = $_SESSION['stno'];
        $getvotequery = "SELECT votedpres FROM studentvote WHERE sno = '$sno'";
        $vote = mysqli_query($conn, $getvotequery);
        $data = mysqli_fetch_array($vote);
        $status = $data['votedpres'];
        if ($status == 0) {
            if (isset($_POST['votepres'])) {
                $id = $_POST['votepres'];
                $votequery = "SELECT votes FROM president WHERE pres_no = '$id'";
                $vote = mysqli_query($conn, $votequery);
                $data = mysqli_fetch_array($vote);
                $getvote = $data['votes'];
                $getvote = $getvote + 1;
                $updatevote = "UPDATE president SET votes = '$getvote' WHERE pres_no = '$id'";
                mysqli_query($conn, $updatevote);
                $updatestudent = "UPDATE studentvote SET votedpres = '$id' WHERE sno = '$sno'";
                mysqli_query($conn, $updatestudent);
                header('Location: vpresi.php');
                exit;
            }
        } else {
            header('Location: vpresi.php');
