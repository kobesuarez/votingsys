...
if (!session_start()) {
    ...
    $sno = $_SESSION['stno'];
    $getvotequery = "SELECT votedgs FROM studentvote WHERE sno = '$sno'";
    $vote = mysqli_query($conn, $getvotequery);
    $data = mysqli_fetch_array($vote);
    $status = $data['votedgs'];
    if ($status == 0) {
        if (isset($_POST['votegensec'])) {
            $id = $_POST['votegensec'];
            $votequery = "SELECT votes FROM gensec WHERE gensec_no = '$id'";
            $vote = mysqli_query($conn, $votequery);
            $data = mysqli_fetch_array($vote);
            $getvote = $data['votes'];
            $getvote = $getvote + 1;
            $updatevote = "UPDATE gensec SET votes = '$getvote' WHERE gensec_no = '$id'";
            mysqli_query($conn, $updatevote);
            $updatestudent = "UPDATE studentvote SET votedgs = '$id' WHERE sno = '$sno'";
            mysqli_query($conn, $updatestudent);
            header('Location: depsec.php');
            exit;
        }
    } else {
        header('Location: depsec.php');
...
<body>
    ...
    <div>
        <?php
        include('connection.php');
        $countquery = "SELECT * FROM candidate WHERE candidateposition = 'General Secretary'";
                    ...
                    <div class="col-6 card text-center" style="width: 18rem;">
                            <img src="src/candidate/General Secretary/' . $imageurl . '" class="card-img-top py-3 rounded-circle" alt="...">
                             <div class="card-body py-0 px-0">
                                ...
                                <button class="btn btn-primary mb-2" type="submit" name = "votegensec" value = "' . $cstno . '" >Vote ' . $cname . ' </button>
                            </div>
                        </div>
                    </div>
                </form>';
